...
- **Small training dataset** - the Mask R-CNN model was trained on 481 images, which limits generalization; performance improves significantly with more labeled data
- **Forecast accuracy** improves as more historical data accumulates - the retraining pipeline (MLflow + feature engineering) is fully in place and ready to scale
- **LLM reports** are generated by a 3B parameter local model - suitable for operational summaries but not expert-level analysis
- **CPU-only inference** in Docker - no GPU required, but image analysis is slower than a GPU-enabled deployment
- **No authentication** - designed for internal facility use on a private network; authentication layer can be added via FastAPI middleware
